...
function M.filename_component()
  ...
  -- Table to store all file names for comparison
  local buffers = {}

  -- Iterate over all buffers
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_loaded(buf) and vim.fn.bufname(buf) ~= "" then
      local file_name = vim.fn.fnamemodify(vim.fn.bufname(buf), ":t")
      if not buffers[file_name] then
        buffers[file_name] = {}
      end
      table.insert(buffers[file_name], buf)
    end
  end

  -- Check if the current file has duplicates
  local current_buf_list = buffers[current_file_name]
  ...
  return table.concat {
    string.format("%%#FilenameSeperator#"),
