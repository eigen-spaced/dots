-- ~/.hammerspoon/init.lua — global hotkeys driving the Emacs daemon.
--
-- A global hotkey fires a one-line elisp call at the always-on Emacs daemon
-- (com.sunny.emacs-daemon LaunchAgent) via `emacsclient' — it just talks to the
-- socket, so no launcher or bash+sleep timing hack is needed.
...
-- Drives the local Spotify app from any context — no API key or Premium needed.
-- smudge in Emacs uses the same AppleScript layer, so the two stay in sync.
...
hs.hotkey.bind(MOD, "N", function()
	local ok, info =
		hs.osascript.applescript('tell application "Spotify" to (name of current track) & " — " & (artist of current track)')
...
-- fresh copy instead of surfacing the running daemon (org.gnu.Emacs).
local appHotkeys = {
	["1"] = { bundleID = "org.gnu.Emacs" },
	["2"] = { bundleID = "com.mitchellh.ghostty" },
	["3"] = { bundleID = "org.mozilla.firefox" },
	["4"] = { bundleID = "com.spotify.client" },
}

-- Bring up the Emacs daemon's frame WITHOUT launching a bare Emacs.app (which
-- ignores `--init-directory'). The daemon is a KeepAlive LaunchAgent, so
-- kickstart is a no-op when it's up, and restarts it with the right config in
-- the brief window right after a manual daemon restart.
local function launchEmacsFrame()
	hs.execute("launchctl kickstart gui/$(id -u)/com.sunny.emacs-daemon", true)
	hs.timer.doAfter(0.3, function()
		hs.task.new(EMACSCLIENT, nil, { "-c", "-n" }):start()
	end)
end

-- Surface the running app (all its windows); launch only if it isn't running.
local function focusOrLaunch(spec)
	local app = hs.application.get(spec.bundleID)
	if app then
		app:activate(true)
		-- Daemon alive but frameless (all frames closed): give it one.
		if
			spec.bundleID == "org.gnu.Emacs"
			and not hs.fnutils.find(app:allWindows(), function(w)
				return w:isStandard()
			end)
		then
			hs.task
	...
	elseif spec.bundleID == "org.gnu.Emacs" then
		launchEmacsFrame()
	else
...
for key, spec in pairs(appHotkeys) do
...
-- ⌥Space opens a HUD listing on-screen windows AND open Emacs file-buffers, each
-- with one key (F → Firefox, c → config.el …); tap it to jump there. `/` drops
-- into a fuzzy search (hs.chooser) over the same list; Escape cancels. Keys come
-- from the entry name (first free letter), digits 1-9 as a fallback.
--
-- Each entry is { name, detail, action }. Windows and Emacs buffers share that
-- shape so the HUD and the fuzzy chooser draw from one list.
...
-- Truncate S to N chars (UTF-8 aware), ellipsis if cut, so long window titles
-- can't blow out the HUD or wrap onto a second line.
local function clip(s, n)
	s = s or ""
	if (utf8.len(s) or #s) <= n then
		return s
	end
...
local LIST_BUFFERS_ELISP = "(mapconcat 'buffer-name (seq-filter 'buffer-file-name (buffer-list)) \"\\n\")"
...
-- Focus an existing Emacs buffer: reuse a visible GUI frame (or make one), give
-- it input focus, switch to the buffer; emacs() then activates the app.
local function emacsSwitchBuffer(name)
	local safe = name:gsub("\\", "\\\\"):gsub('"', '\\"')
	emacs(
		"(let ((f (or (seq-find 'frame-visible-p (frame-list)) (make-frame))))"
...
-- `/` from the HUD: fuzzy-search the same entries via hs.chooser, which owns all
-- typing — so single-key and search modes can't collide.
local function switchFuzzy()
	...
	local chooser = hs.chooser.new(function(choice)
		if choice and switchEntries[choice.idx] then
			switchEntries[choice.idx].action()
		end
	end)
...
local function switchBuild(buffers)
	...
	for _, win in ipairs(hs.window.orderedWindows()) do
		...
		-- so the same file isn't listed twice. No buffers → fall back to the window.
	...
	for _, r in ipairs(rows) do
		namew = math.max(namew, #r.name)
	end
	for _, r in ipairs(rows) do
		lines[#lines + 1] = string.format("[%s]  %-" .. namew .. "s   %s", r.key, r.name, clip(r.detail, 52))
	end
...
-- init.lua is a symlink into ~/dotfiles; FSEvents reports edits against the REAL
-- path, so the watcher above never sees them. Also watch the real dir.
